...
export const aqq = (enable?: any, ...args: any) =>
    enable ? console.log("aqq", ...args) : void 0;

/** JSON stringify */
export const stringify: (src: any, spaceChars?: number) => string = (
    src,
    spaceChars,
...
export const _range: (src: number) => number[] = (src) =>
    new Array(Math.round(Number(src))).fill(0).map((_, i) => i);

export const _round: (src: number, fractionSigns?: number) => number = (
    src,
    fractionSigns,
) =>
    fractionSigns
        ? Math.round(src * 10 ** fractionSigns) / 10 ** fractionSigns
        : Math.round(Number(src));
...
export const _prettifyNumber = (src: number, maxChars = 2): string => {
    ...
    return charDiff > 0
        ? `${_range(charDiff).fill(0).join("")}${src}`
        : String(src);
};
